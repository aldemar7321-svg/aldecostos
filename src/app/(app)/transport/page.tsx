...
const TransportContent = () => {
  const { transport, addTransportItem, updateTransportItem, deleteTransportItem, laborSettings } = useAppData();
  ...
  const handleDelete = () => {
    if (itemToDelete) {
      deleteTransportItem(itemToDelete);
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    if (editingItem) {
      updateTransportItem({ ...editingItem, ...values });
    } else {
  ...
  const totalTransportCost = transport.reduce(
    (acc, item) => acc + item.monthlyValue * item.productionPercentage,
    0
  );
  const transportRate =
    laborSettings.totalMonthlyHours > 0
      ? totalTransportCost / laborSettings.totalMonthlyHours
      : 0;
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        description="Gestiona los gastos de transporte y su asignación a producción."
      >
      ...
      <Card>
        <CardHeader>
          <CardTitle>Gastos Mensuales de Transporte</CardTitle>
          <CardDescription>
            El costo total se calcula aplicando el '% Producción' al valor mensual
            de cada concepto.
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <TableBody>
              ...
              {transport.length === 0 && (
                <TableRow>
                  ...
                  >
                    No has añadido ningún concepto de transporte.
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
        ...
        <CardFooter className="flex-col items-end gap-2 sm:flex-row sm:justify-end sm:items-center">
          <div className="flex flex-col items-end">
            <p className="text-muted-foreground">Costo Total de Transporte:</p>
            <p className="text-xl font-bold">{formatCurrency(totalTransportCost)}</p>
          </div>
          <div className="h-12 w-px bg-border hidden sm:block mx-4"></div>
          <div className="flex flex-col items-end">
            <p className="text-muted-foreground">Tasa de Transporte (por hora de M.O.):</p>
            <p className="text-xl font-bold">{formatCurrency(transportRate)} / hora</p>
          </div>
        </CardFooter>
      </Card>
      <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
        <SheetContent>
          <SheetHeader>
            <SheetTitle>{editingItem ? 'Editar Concepto' : 'Añadir Nuevo Concepto de Transporte'}</SheetTitle>
            <SheetDescription>
            {editingItem ? 'Modifica los detalles del concepto.' : 'Completa los detalles del nuevo costo de transporte.'}
            </SheetDescription>
          </SheetHeader>
          <Form {...form}>
            <form
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Ej: Envío Local" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
