...
import { z } from 'zod';
import type { OverheadItem, AllocationBasis } from '@/lib/types';
...
export default function OverheadPage() {
  const { overhead, addOverheadItem, updateOverheadItem, deleteOverheadItem } = useAppData();
  ...
  const handleDialogOpen = (item: OverheadItem | null = null) => {
    setEditingItem(item);
    if (item) {
      form.reset({
  ...
  const onSubmit = (data: FormData) => {
    if (editingItem) {
      updateOverheadItem({ ...editingItem, ...data });
       toast({ title: "Costo actualizado", description: `"${data.concept}" ha sido actualizado.` });
    } else {
      addOverheadItem(data);
      toast({ title: "Costo añadido", description: `"${data.concept}" ha sido añadido.` });
    }
    handleDialogClose();
  };
  ...
  const handleDelete = () => {
    if (deletingItemId) {
      const itemToDelete = overhead.find(item => item.id === deletingItemId);
      deleteOverheadItem(deletingItemId);
      setIsAlertOpen(false);
      setDeletingItemId(null);
      toast({ title: "Costo eliminado", description: `"${itemToDelete?.concept}" ha sido eliminado.` });
    }
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Gestiona los costos indirectos de fabricación (CIF)."
      >
            ...
            {overhead.length > 0 ? (
              ...
              <TableRow>
                <TableCell colSpan={6} className="text-center h-24">
                  No hay costos indirectos definidos.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[425px]" onInteractOutside={handleDialogClose}>
          <DialogHeader>
            <DialogTitle>{editingItem ? 'Editar Costo' : 'Añadir Costo Indirecto'}</DialogTitle>
            <DialogDescription>
              {editingItem
                ? 'Actualiza los detalles del costo.'
                : 'Añade un nuevo costo a tu lista de CIF.'}
            </DialogDescription>
          </DialogHeader>
          <Form {...form}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Ej: Arriendo de bodega" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input type="number" placeholder="Ej: 1500000" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input type="number" step="0.01" min="0" max="1" placeholder="Ej: 0.8 (para 80%)" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
      ...
      <AlertDialog open={isAlertOpen} onOpenChange={setIsAlertOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta acción no se puede deshacer. Esto eliminará permanentemente el costo indirecto.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
