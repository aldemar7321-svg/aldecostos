...
import { z } from 'zod';
import type { TransportItem, AllocationBasis } from '@/lib/types';
...
export default function TransportPage() {
  const { transport, addTransportItem, updateTransportItem, deleteTransportItem } = useAppData();
  ...
  const handleDialogOpen = (item: TransportItem | null = null) => {
    setEditingItem(item);
    if (item) {
      form.reset({
  ...
  const onSubmit = (data: FormData) => {
    if (editingItem) {
      updateTransportItem({ ...editingItem, ...data });
       toast({ title: "Costo de transporte actualizado", description: `"${data.concept}" ha sido actualizado.` });
    } else {
      addTransportItem(data);
      toast({ title: "Costo de transporte añadido", description: `"${data.concept}" ha sido añadido.` });
    }
    handleDialogClose();
  };
  ...
  const handleDelete = () => {
    if (deletingItemId) {
      const itemToDelete = transport.find(item => item.id === deletingItemId);
      deleteTransportItem(deletingItemId);
      setIsAlertOpen(false);
      setDeletingItemId(null);
      toast({ title: "Costo de transporte eliminado", description: `"${itemToDelete?.concept}" ha sido eliminado.` });
    }
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Transporte"
        description="Gestiona los costos asociados al transporte y distribución."
      >
            ...
            {transport.length > 0 ? (
              ...
              <TableRow>
                <TableCell colSpan={6} className="text-center h-24">
                  No hay costos de transporte definidos.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[425px]" onInteractOutside={handleDialogClose}>
          <DialogHeader>
            <DialogTitle>{editingItem ? 'Editar Costo' : 'Añadir Costo de Transporte'}</DialogTitle>
            <DialogDescription>
              {editingItem
                ? 'Actualiza los detalles del costo.'
                : 'Añade un nuevo costo a tu lista de transporte.'}
            </DialogDescription>
          </DialogHeader>
          <Form {...form}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Ej: Envío a clientes" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input type="number" placeholder="Ej: 300000" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
      ...
      <AlertDialog open={isAlertOpen} onOpenChange={setIsAlertOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta acción no se puede deshacer. Esto eliminará permanentemente el costo de transporte.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
